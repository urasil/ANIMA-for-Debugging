...
from src.custom_synthesis import synthesis, transfer_voice, trim_silence, generateAnimaProfile, synthesisFromProfile
...
class Synthesizer(object):
    def __init__(
        ...
        print("output sample rate = ", str(self.output_sample_rate))
    ...
    def _load_tts(self, tts_checkpoint: str, tts_config_path: str, use_cuda: bool) -> None:
        ...
        print("getting from : ", tts_config_path)
    ...
    def createProfile(
        ...
        """ Generate model and save all data to an [file_name].animaprofile file

        Args:
            file_path (str): File path to created file.
        ...
        if not file_path:
            raise ValueError(
                "You need to define `file_path` to create a profile file."
            )
    ...
    def ttsFromProfile(
        ...
        """🐸 TTS magic. Run model from saved ANIMA profile and generate speech. 

        Args:
            profile_path (str): path to the ANIMA profile file.
            text (str): input text.
        Returns:
        ...
        if not text or not profile_path:
            raise ValueError(
                "You need to define `text` and `profile_path` for sythesis."
            )
